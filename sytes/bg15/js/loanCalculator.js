$(document).ready(function () {
    ...
    timeTable.text(time);
    commissionTableSize.text(commission.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")).append(" рублей");
    returnTable.text(returnTotal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")).append(" рублей");
    probabilityTable.text(probability).append('%');
    range.on("change", function () {
        ...
        rangeTableValue.text(currentLoanToShow).append(" рублей");
        commissionTableSize.text(commissionToShow).append(" рублей");
        returnTable.text(totalToShow).append(" рублей");
